...
describe('Read Receipts Store', () => {
  ...
  describe('Read Receipts Management', () => {
    it('should mark message as read', async () => {
      ...
      expect(mockSocketService.getInstance().emit).toHaveBeenCalledWith(
        'MESSAGE_READ',
        {
          messageId,
          roomId,
  ...
  describe('Lifecycle Management', () => {
    it('should initialize correctly', async () => {
      ...
      expect(mockSocketService.getInstance().on).toHaveBeenCalledWith('READ_RECEIPT_UPDATED', expect.any(Function))
      expect(mockSocketService.getInstance().on).toHaveBeenCalledWith('NOTIFICATION_RECEIVED', expect.any(Function))
    })
    ...
    it('should cleanup resources on destroy', () => {
      ...
      expect(store.state.notifications.size).toBe(0)
      expect(mockSocketService.getInstance().off).toHaveBeenCalledWith('READ_RECEIPT_UPDATED', expect.any(Function))
      expect(mockSocketService.getInstance().off).toHaveBeenCalledWith('NOTIFICATION_RECEIVED', expect.any(Function))
    })
  })
})
