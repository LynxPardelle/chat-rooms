...
export class SecurityMiddleware implements NestMiddleware {
   ...
   */
  private async checkRateLimit(req: Request): Promise<void> {    // This would integrate with existing rate limiting service
    // For now, we'll add basic monitoring
  ...
  private async prepareSecureDbContext(req: Request): Promise<void> {
    ...
    req['securityContext'] = {
      ...
      userId: (req['user'] as any)?.username || (req['user'] as any)?.email,
      ip: req.ip,
      userAgent: req.get('User-Agent'),
      timestamp: new Date(),
    };
   ...
   * Log security incident
   */
  private async logSecurityIncident(req: Request, error: Error): Promise<void> {    await this.securityMonitoring.reportSecurityEvent(
      'API_ABUSE', // Use valid SecurityEventType
      'SecurityMiddleware',
      {
